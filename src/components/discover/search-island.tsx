...
export function DiscoverSearch({
  ...
  const buildParams = useCallback(
    (resolved: PlaceDto) => {
      ...
      if (distance === "custom") {
        params.set("radiusKm", String(customRadiusKm));
      }
      return params;
    },
  ...
  const navigateWith = useCallback(
    (resolved: PlaceDto, replace = false) => {
      const url = `${basePath}?${buildParams(resolved).toString()}${hash || ""}`;
