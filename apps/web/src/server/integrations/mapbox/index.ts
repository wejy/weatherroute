...
export type MapboxRoutesResult = {
  /** Directions API `code` (e.g. Ok, NoRoute, NoSegment), or NoToken when unset. */
  code: string;
  routes: MapboxRoute[];
};
...
export async function getMapboxRoutes(
  ...
  if (!hasMapbox()) return { code: "NoToken", routes: [] };
