...
export function buildWeatherAdvisories(
  ...
  if (rainProbability >= 50 && condition !== "storm") {
    out.push({
      id: "rain",
      tone: "warning",
      icon: "umbrella",
  ...
  } else if (rainProbability >= 30 && condition !== "storm" && condition !== "rainy") {
    // rainy condition already implies wet; still flag moderate chance when cloudy
    out.push({
      id: "rain-moderate",
      tone: "caution",
      icon: "water_drop",
